...
class MarkdownBuilder implements md.NodeVisitor {
  ...
  void visitElementAfter(md.Element element) {
    ...
    if (_isBlockTag(tag)) {
      ...
      Widget child = current.children.isNotEmpty
          ? Column(
      ...
      if (_isListTag(tag)) {
        ...
        if (_listIndents.isNotEmpty && element.children!=null) {
      ...
      parent.children.add(_mergeInlineChildren(current.children));
    }
  }

  void _addAnonymousBlockIfNeeded() {
    if (_inlines.isNotEmpty) {
      _blocks.last.children.add(_mergeInlineChildren(_inlines.last.children));
      _inlines.clear();
    }
  }
  ...
  void _addParentInlineIfNeeded(String? tag) {
    if (_inlines.isEmpty) {
      TextStyle? parentStyle = styleSheet.styles[tag];
      _inlines.add(_InlineElement(tag, style: parentStyle));
    }
  }

  void _addBlockChild(Widget child) {
    _blocks.last.children.add(child);
  }

  Widget _mergeInlineChildren(List<Widget> children) {
  ...
  Widget _buildRichText(TextSpan text) {
    return selectable ? SelectableText.rich(text) : RichText(text: text);
  }

  Widget _buildBullet(String listTag) {
    if (listTag == 'ul') {
      return Text(styleSheet.listBullet);
    } else {
      final int index = _blocks.last.nextListIndex;
      _blocks.last.nextListIndex += 1;
      return Text('$index.');
    }
  }
  ...
  Widget _buildCheckbox(bool checked) {
    ...
    return Padding(
      padding: styleSheet.checkboxPadding!,
      child: Icon(
  ...
  Widget _buildImage(String src) {
    if (imageBuilder != null) {
      return imageBuilder!(Uri.parse(imageDirectory + src));
    } else {
      return Image.network(imageDirectory + src);
    }
  }

  Widget _buildTableCell(Widget child, {TextAlign? textAlign}) {
    return Padding(
...
class MarkdownBuilder implements md.NodeVisitor {
  ...
  void visitText(md.Text text) {
    ...
    if (child != null) _inlines.last.children.add(child);
