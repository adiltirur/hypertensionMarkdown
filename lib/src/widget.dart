...
class _MarkdownWidgetState extends State<MarkdownWidget>
  ...
  GestureRecognizer createLink(String text, String? href, String title) {
    final TapGestureRecognizer recognizer = TapGestureRecognizer()
      ..onTap = () async {
        final newHref = href ?? '';
        if (newHref.contains('www.')) {
          OpenBrowser.launchURL(newHref);
        } else {
          var date;
          var article = widget.language == 'en'
              ? articlesEN[int.parse(href!)]
              : articles[int.parse(href!)];
          date = await ReadDate.getDate(article["rank"]);
